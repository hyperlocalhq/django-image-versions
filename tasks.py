...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-image-versions runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-image-versions.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ image_versions")
...
def lint(c):
    ...
    """
    c.run("flake8 django-image-versions tests")
...
def release(c, bumpsize=''):
    ...
    import django_image_versions
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_image_versions.__version__))
    c.run("git push --tags")
